...
if (!$data || !$numpages) {
    ...
    if ($page <= $numpages) {
        ...
        // Check if the width exceeds 1600 pixels and resize proportionally if necessary
        if ($width > 1600) {
            $newHeight = ($height * 1600) / $width;
            $im->resizeImage(1600, $newHeight, Imagick::FILTER_LANCZOS, 1);
        }
    ...
    $im->destroy();
         ...
         echo $OUTPUT->header();
        \core\notification::error("No Valid Files Found!");
        echo $OUTPUT->footer();
        die();
