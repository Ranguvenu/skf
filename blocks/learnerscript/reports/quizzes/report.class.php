...
class report_quizzes extends reportbase implements report {
    ...
    public function __construct($report, $reportproperties) {
        ...
        $this->parent = true;
        $this->columns = ['quizfield' => ['quizfield'] , 'quizzes' => [
        ...
        $this->orderable = ['avggrade', 'grademax', 'gradepass', 'notattemptedusers',
        'inprogressusers', 'completedusers', 'noofcompletegradedfirstattempts', 'totalattempts',
        'totalnoofcompletegradedattempts', 'avggradeoffirstattempts',
        'avggradeofallattempts', 'avggradeofhighestgradedattempts', 'totaltimespent', 'name', 'course', ];
        $this->searchable = ['main.name', 'c.fullname', 'c.shortname'];
    ...
    public function from() {
        $this->sql .= " FROM {quiz} main";
    }
    /**
     * SQL JOINS
     */
    public function joins() {
        $this->sql .= " JOIN {course_modules} cm ON cm.instance = main.id
        ...
        switch ($columnname) {
            ...
            case 'noofcompletegradedfirstattempts':
                ...
                $query = "SELECT COUNT(*) AS noofcompletegradedfirstattempts
                        ...
                        AND quiza.state = 'finished' AND (quiza.state = 'finished'
                        AND NOT EXISTS ( SELECT 1 FROM {quiz_attempts} qa2
                        WHERE qa2.quiz = quiza.quiz AND qa2.userid = quiza.userid
                        AND qa2.state = 'finished' AND qa2.attempt < quiza.attempt))
                        AND quiza.userid IN ($learnersql) AND quiza.sumgrades IS NOT NULL $where ";
            break;
            case 'totalnoofcompletegradedattempts':
                ...
                $query = "SELECT COUNT(*) AS totalnoofcompletegradedattempts
                              FROM {quiz_attempts} quiza
                              JOIN  {quiz} q ON quiza.quiz = q.id
                              WHERE quiza.quiz = q.id AND quiza.preview = 0 AND quiza.state = 'finished'
                              AND quiza.userid IN ($learnersql) AND quiza.sumgrades IS NOT NULL $where ";
            break;
            case 'avggradeofhighestgradedattempts':
                ...
                $query = "SELECT CASE WHEN q.sumgrades > 0 THEN
                        (CASE WHEN AVG(quiza.sumgrades) > 0 THEN AVG(quiza.sumgrades) * 100 / q.sumgrades ELSE 0 END)
                        ELSE 0 END AS avggradeofhighestgradedattempts
                        FROM {quiz_attempts} quiza
                        JOIN  {quiz} q ON quiza.quiz = q.id
                        WHERE quiza.quiz = q.id AND quiza.userid IN ($learnersql)
                        AND quiza.preview = 0 AND quiza.state = 'finished' AND (quiza.state = 'finished'
                        AND NOT EXISTS ( SELECT 1 FROM {quiz_attempts} qa2
                        WHERE qa2.quiz = quiza.quiz AND qa2.userid = quiza.userid AND qa2.state = 'finished'
                        AND ( COALESCE(qa2.sumgrades, 0) > COALESCE(quiza.sumgrades, 0)
                        OR (COALESCE(qa2.sumgrades, 0) = COALESCE(quiza.sumgrades, 0)
                        AND qa2.attempt < quiza.attempt) ))) AND quiza.sumgrades IS NOT NULL
                        $where GROUP BY q.sumgrades ";
            break;
            case 'avggradeoffirstattempts':
                ...
                $query = "SELECT CASE WHEN q.sumgrades > 0 THEN
                        (CASE WHEN AVG(quiza.sumgrades) > 0 THEN AVG(quiza.sumgrades) * 100 / q.sumgrades ELSE 0 END)
                        ELSE 0 END AS avggradeoffirstattempts
                        FROM {quiz_attempts} quiza
                        JOIN  {quiz} q ON quiza.quiz = q.id
                        WHERE quiza.quiz = q.id
                        AND quiza.preview = 0 AND quiza.userid IN ($learnersql)
                        AND quiza.state = 'finished' AND (quiza.state = 'finished'
                        AND NOT EXISTS ( SELECT 1 FROM {quiz_attempts} qa2
                        WHERE qa2.quiz = quiza.quiz AND qa2.userid = quiza.userid
                        AND qa2.state = 'finished' AND qa2.attempt < quiza.attempt))
            ...
            case 'avggradeofallattempts':
                ...
                $query = "SELECT CASE WHEN q.sumgrades > 0 THEN
                        (CASE WHEN AVG(quiza.sumgrades) > 0 THEN AVG(quiza.sumgrades) * 100 / q.sumgrades ELSE 0 END)
                        ELSE 0 END AS avggradeofallattempts
                        FROM {quiz_attempts} quiza
                        JOIN  {quiz} q ON quiza.quiz = q.id
                        WHERE quiza.quiz = q.id AND quiza.userid IN ($learnersql)
                        AND quiza.preview = 0 AND quiza.state = 'finished'
                        AND quiza.sumgrades IS NOT NULL $where GROUP BY q.sumgrades";
            break;
            ...
            case 'notattemptedusers':
                ...
                $query = "SELECT COUNT(DISTINCT u.id) AS notattemptedusers
                           ...
                           WHERE  u.id NOT IN ( SELECT qat.userid
                             ...
                             AND u.id > 2 AND u.deleted = 0 AND u.confirmed = 1 AND u.suspended = 0
                             AND ra.userid IN ($learnersql) $where ";
            break;
            case 'totaltimespent':
                $identy = 'cm.instance';
            ...
            case 'numviews':
                ...
                if ($this->reporttype == 'table') {
                    $query = "  SELECT COUNT(DISTINCT lsl.userid) as distinctusers, COUNT('X') as numviews
                                 ...
                                 AND lsl.userid > 2  AND u.confirmed = 1 AND u.deleted = 0
                                 AND lsl.anonymous = 0 AND m.name = 'quiz'
                    ...
                    $query = "  SELECT COUNT('X') as numviews
                                 ...
                                 AND u.deleted = 0 AND m.name = 'quiz'
                                 AND lsl.target = 'course_module' $where";
                }
            break;
            default:
