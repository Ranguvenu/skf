...
class observer {
    ...
    public static function store(\core\event\base $event) {
        global $CFG, $DB, $USER;
        if (!is_siteadmin()) {
            $browscap = new \block_configurable_reports_browscap($CFG->dataroot . '/cache/');
    ...
    public static function ls_timestats() {
        ...
        $insertdata->timespent = isset($_COOKIE['time_timeme_cr']) ? round($_COOKIE['time_timeme_cr']) : '';
        ...
        $insertdata1->timespent = isset($_COOKIE['time_timeme_cr']) ? round($_COOKIE['time_timeme_cr']) : '';
        // print_r($_COOKIE['time_timeme_cr']);

        if (isset($_COOKIE['time_timeme_cr']) && isset($_SESSION['pageurl_timeme']) &&
            $_COOKIE['time_timeme_cr'] != 0) {
            // print_r($insertdata1);exit;

            $record1 = $DB->get_record('block_cr_coursetimestats',
        ...
        $PAGE->requires->js_call_amd('block_configurable_reports/track', 'timeme');
        // exit;
        $_COOKIE['time_timeme_cr'] = 0;
        unset($_COOKIE['time_timeme_cr']);
    }
